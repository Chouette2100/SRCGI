...
ェックを完了してくだ>さい。</p>
	...
	<form>
		...
		<p style="padding-left:2em">SHOWROOMイベント情報ページからDBへのイベント参加ルーム情報の追加と更新
			...
			*/}}
			<font color="red">現在この機能は使えない場合があります。その場合は...<br>
					取得範囲を広げるには下の「イベントパラメータの設定」をお使いください。<br>
					一覧にないルームを追加するには上の「(DB登録済み)イベント参加ルーム一覧（確認・編集）」をお使いください。<br>
					お手数をおかけしますがよろしくお願いします。</font>
		</p>

		<p style="padding-left:6em">
			<label>ＤＢに登録する順位の範囲
					<input type="number" name="breg" value="{{.Fromorder}}" size="3" required min="1" max="50"><label>位から
					<input type="number" name="ereg" value="{{.Toorder}}" size="3" required min="1" max="50"><label>位まで　　　
