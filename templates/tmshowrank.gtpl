...
<body>
   ...
   <div style="text-indent: 2rem;"><a href="https://chouette2100.com/">記事/ソース/CGI一覧表</a>　（証明書の期限が切れてしまっていました。2022年8月29日、有効な証明書に切り替えました）</div>
    ...
    <p style="color: black;">
    こちらは月始めのデータです。<br>
    「月初のSHOWランク」というのは毎月1日の0時45分から15分ほどかけて取得したデータをいいます<br>
    （「SHOWランク上位のルーム」の表の「DBに登録済みの配信者のうちSHOWランク上位220ルーム」に相当します<br>
    　これらはすべて同一のタイムスタンプ（＝00:45）になります<br>
    ...
    <table>
        ...
        <tr bgcolor="{{$c}}" >
            ...
            <a href="closedevents?userno={{ .Userno }}&mode=0&path=5">{{ .User_name }} <a href="/onlives?roomid={{.Userno}}">.</a></a>
            </td>
