...
type ListLastC struct {
	Eventid         string // header
	Ieventid        string
	Userno          string
	UpdateTime      string
	NextTime        string
	...
	TurnstileSiteKey string // Turnstile導入用(1)
	TurnstileError   string
	RequestID        string
}

var ListLastCFuncMap *template.FuncMap = &template.FuncMap{
	//	3桁ごとに","を挿入する
...
func (h *ListLastC) SetTurnstileInfo(siteKey string, errorMsg string) {
	h.TurnstileSiteKey = siteKey
	h.TurnstileError = errorMsg
}

func (h *ListLastC) GetTemplatePath() string {
	return "templates/listlastp.gtpl"
}

func (h *ListLastC) GetTemplateName() string {
	return "listlastp.gtpl"
}

func (h *ListLastC) GetFuncMap() *template.FuncMap {
	return nil
}

// -------------------------------------------

func ListLastcHandler(w http.ResponseWriter, req *http.Request) {
...
func SelectCurrentScorec(
	...
	err = Db0.QueryRow(sql0, eventid, eventid).Scan(&totalCount)
